...
void Adafruit_SSD1306::begin(uint8_t vccstate, uint8_t i2caddr, bool reset) {
  ...
  if (sid != -1){
    ...
    dcport      = (volatile unsigned char*)portOutputRegister(digitalPinToPort(dc));
    ...
    if (!hwSPI){
      ...
      pinMode(sclk, OUTPUT);
#ifdef HAVE_PORTREG
      clkport     = (volatile unsigned char*)portOutputRegister(digitalPinToPort(sclk));
      clkpinmask  = digitalPinToBitMask(sclk);
      mosiport    = (volatile unsigned char*)portOutputRegister(digitalPinToPort(sid));
